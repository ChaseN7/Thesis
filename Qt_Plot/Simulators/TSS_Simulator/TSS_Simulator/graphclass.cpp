...
/// 1. Check whether a Mutant from left could kill the chosen
/// 2. Check whether a Mutant from right could kill the chosen
/// If non can, than we are STUCK! -> return true;
bool GraphClass::isStuck() const
{
    const int chosen = Manager.Events.ChosenTrait;
    if(chosen > 0){
        if(TraitClass::Fitness[chosen - 1][chosen] > 0)
            return false;
    }
