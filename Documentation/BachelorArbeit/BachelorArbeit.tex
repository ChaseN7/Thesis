...
Ziel wird es sein zwei spezielle BPDL-Grenzwert-Prozesse simulieren zu k"onnen.
	...
	\textit{Sei $ X $ der Raum der Merkmale. Jedes Individuum hat genau ein solches Merkmal $ x \in X $. Wenn es hilfreich ist kann man sich X als Indexmenge: $ X = \{1,\dots, n\} $ vorstellen, die eine Durchz"ahlung der Merkmale darstellt. Das entspricht auch der Interpretation von X aus Sicht der Simulation.\\
	Dann gilt f"ur jedes Individuum mit Merkmal $ x \in X $:
	\begin{itemize}
		...
		\item Die Fortpflanzung des Individuums kann in der Geburt eines Individuums mit Merkmal $ y \in X $ (Mutation) resultieren. Die H"aufigkeit dieser Ereignisse wird durch die Mutationswahrscheinlichkeit beschrieben.
	\end{itemize}}
	...
	\begin{tabular}{ r p{18em} }
		...
		$ d(x) + \sum_{y=1}^{n} c(x,y) \cdot n_t(y) $ & Wie oben, nur mit $ n \hat{=} $ \#Merkmale, und $ n_t(x) \hat{=}$ \#Individuen zur Zeit t mit Merkmal $ x $
	\end{tabular}\\
	...
	\textit{	Diese beschreibt die Rate mit der Fortpflanzungen innerhalb des Merkmals x stattfinden (nicht die Geburten innerhalb x!)}\\
		\textit{\item Die arteigene Geburtenrate(\textbf{Wachstumsrate}) des Merkmals $ x $ ist von besonderem Interesse und ist das womit wir folgend haupts"achlich arbeiten werden: 
		\begin{align*}
	...
	Eine auf diesem Modell basierende Population ist eine Zufallsvariable mit folgendem \textcolor{blue}{Ma"s}:
	...
	\[ \nu_t \in M(X) = \left\{ \sum_{i=1}^{n} \delta_{x_i}, n \in \mathbb{N}, x_1, \dots, x_n \in X \right\} \]
	...
	\end{align*}
	mit $ \phi: M_F \to \mathbb{R} $. Dieser beschreibt die erwartete "Anderung von $ \nu $ zur Zeit $ t $. Man erkennt dass der Generator unabh"angig von $ t $ ist, da er nur mit Zeitunabh"angigen Parametern $ b, d, c, \mu $ konstruiert wurde. Nat"urlich ist der verwendete Prozess $ \nu_t $ abh"angig von t, weshalb man $ \frac{d}{dt}\mathbb{E}\phi(\nu_t) = L\phi(\nu_t) $ schreiben kann.\\
\textit{	Mit unserem diskreten Raum X und der konstanten Mutationswahrscheinlichkeit zu Nachbarn vereinfacht sich der Generator. Der f"ur unser Model angepasster Generator hat somit folgende Form:
	\begin{align*}
	...
	\end{align*}
	mit $ \phi: M \to \mathbb{R} $ und $ n(x_i) \hat{=} $ \#Individuen mit Merkmal $ x_i $.\\	}
...
\subsection{Normalisierung des BPDL Prozesses}
	...
	Die Raten für Geburten und natürliche Tode der Individuen bleiben unverändert. Da die Populationsgröße jedoch quadratisch in Wettbewerbsrate einfließt, sollte $ c_K = \frac{c}{K} $ gelten, da sonst die K-fach erh"ohte Population mit einem intensiven Aussterben den Vergleich verf"alschen w"urde. \\
	...
	\begin{align*}
		L^K(\phi(\nu)) &= \int_{X} b(x)(1-\mu)\left[\phi\left(\nu + \frac{\delta_x}{K}\right) - \phi(\nu)\right]K\nu(dx)\\
			  &+ \int_{X}\int_{\mathbb{R}^d} b(x) \cdot \mu \left[\phi\left(\nu + \frac{\delta_{x+z}}{K}\right) - \phi(\nu)\right] m(x,dz) K \nu(dx)\\
		  	  &+ \int_{X} d(x)\left[\phi\left(\nu - \frac{\delta_x}{K}\right) - \phi(\nu)\right]K\nu(dx)\\
		 	  &+ \int_{X} \left( \int_{X} \textcolor{blue}{c_K(x,y)} \nu(dy) \right) \left[\phi(\nu - \frac{\delta_x}{K}) - \phi(\nu)\right]K\nu(dx)
	\end{align*}
	und in unserem Fall zu:
	\begin{align*}
		L^K(\phi(\nu)) &= \sum_{x \in X} b(x)(1-\mu)\left[\phi\left(\nu + \frac{\delta_x}{K}\right) - \phi(\nu)\right]K \cdot n(x)\\
		&+ \sum_{y \sim x}b(x) \cdot \mu \cdot 
	 \left[\phi\left(\nu + \frac{\delta_y}{K}\right) - \phi(\nu)\right]K \cdot n(x)\\		 
		&+ \sum_{x \in X} \left(d(x) + \sum_{y \in X} c_K(x,y) \cdot n(y)\right)\left[\phi\left(\nu - \frac{\delta_x}{K}\right) - \phi(\nu)\right]K \cdot n(x)\\
	\end{align*}
	
	Wie man vermuten kann konvergiert das System $ \nu_t^K $ mit $ K \to \infty $ gegen ein deterministisches System. Um das nachzuweisen reicht es die Bedingungen f"ur \cite[\textbf{Thm 2.1}]{ethier2009markov}, also 2.6 und 2.7 zu pr"ufen: [...]\\
%	(2.6) ist in unserem Fall einfach, da wir stets endlich viele Merkmale mit endlichen Raten betrachten. \\
%	(2.7) ???\\
%	(2.8) und (2.3) sind per Definition so gew"ahlt worden.
	%Ein weiteres Beispiel dazu ist Abbildung (\ref{Konvergenz_K=100000}).\\
	%Wie man bereits erkennt konvergiert f"ur $ K \to \infty $ der Prozess zu einer deterministischen Funktion der die 
	\\Für eine Anfangspoulation die aus zwei Merkmalen bestehet, konvergiert der Prozess ohne Mutation gegen die Lösung der Konkurrenz Lotka-Volterra Gleichung für zwei Spezies (Siehe Kapitel 11 -Law of Large Numbers- von Ethier und Kurtz \cite{ethier2009markov}).
	...
	\end{split}
	\end{align}
	Wir wollen hieraus einen stabilen Zustand f"ur die Population ermitteln indem sich die Populationsgr"o"se nicht mehr "andern darf:
	\begin{align}
		0 &= \dot{n} = (b(x) - d(x) - nc(x,x))n \nonumber\\
		\Rightarrow 0 &= b(x) - d(x) - nc(x,x), \qquad n \ne 0\nonumber\\
		\Rightarrow \bar{n} &= \frac{b(x) - d(x)}{c(x,x)} \label{monorphEquilibrium}
	\end{align}
	...
	Ab jetzt wird mit $ \bar{n}_x $ der monomorphe Gleichgewichtszustand aus (\ref{monorphEquilibrium}) gemeint.
	...
	\subsection{Gleichgewicht im dimorphen Fall}
		Wir wissen mittlerweile dass die Population für $ K \to \infty $ gegen ein deterministisches System ($ n_1, \dots, n_n $) konvergiert. Angenommen es gibt wieder keine Mutation, dann gilt:\\
		\begin{align}
		\begin{split}
			\dot{n}_x & = n_x (b(x) - d(x) - c(x,x)n_x - c(y,x) n_y) \quad n_x(0) = n_{x,0} \\
	...
	\[ \left( \frac{1}{e^{VK}} \ll \mu_K \ll \frac{1}{K log(K)} \right), \] 
